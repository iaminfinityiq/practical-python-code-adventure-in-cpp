...
int main() {
    ...
    for (CharactersStruct &character : characters) { // Iterates each character
        if (character.characters == 3) { // Checks if the streak is exactly 3
            count++; // Increments count
        }
    }

    std::cout << count; // Our result is here
    return 0;
}